# run_backtest.py
#!/usr/bin/env python3
"""
For every folder in children/, create a self-contained Lean project
and launch a cloud back-test. Stores {run_name: backtestId} in backtests.json.

Requires GitHub secrets:
  QC_PROJECT_ID
"""

from __future__ import annotations
import json, os, pathlib, shutil, subprocess, sys, uuid

ROOT       = pathlib.Path(__file__).resolve().parent
CHILD_DIR  = ROOT / "children"
OUT_FILE   = ROOT / "backtests.json"
PROJECT_ID = os.getenv("QC_PROJECT_ID")

if not PROJECT_ID:
    sys.exit("QC_PROJECT_ID env var missing")

ESSENTIALS = ["main.py", "strategies"]   # copied into every child
records: dict[str, str] = {}

for child in CHILD_DIR.iterdir():
    if not child.is_dir():
        continue

    # 1️⃣  copy main.py & strategies/ into the variant folder
    for item in ESSENTIALS:
        src = ROOT / item
        dst = child / item
        if dst.exists():
            shutil.rmtree(dst) if dst.is_dir() else dst.unlink()
        shutil.copytree(src, dst) if src.is_dir() else shutil.copy2(src, dst)

    # 2️⃣  run back-test from inside that folder
    run_name = f"{child.name}-{uuid.uuid4().hex[:6]}"
    print(f"🚀  {run_name}")
    cmd = [
        "lean", "cloud", "backtest", PROJECT_ID,
        "--push",
        "--name", run_name,
        "--wait",
        "--output", "backtest.json",
    ]
    proc = subprocess.run(cmd, cwd=child, text=True,
                          capture_output=True, timeout=900)

    if proc.returncode:
        print(f"❌  {run_name}:\n{proc.stderr or proc.stdout}")
        continue

    try:
        bt_id = json.loads((child / "backtest.json").read_text())["backtestId"]
        records[run_name] = bt_id
        print(f"✔   {run_name}  →  {bt_id}")
    except Exception as exc:
        print(f"⚠️   {run_name}: couldn’t parse backtestId ({exc})")

OUT_FILE.write_text(json.dumps(records, indent=2))
print(f"📝  Back-test IDs saved to {OUT_FILE.relative_to(ROOT)}")
