...
Steps
  1.  copy main.py & strategies/ into children/* so each folder is a
...
for child in CHILD_DIR.iterdir():
    ...
    # 2️⃣ run back-test (CLI 1.x: no --wait / --json / --output)
    run_name = f"{child.name}-{uuid.uuid4().hex[:6]}"
    print(f"🚀  {run_name}")
    cmd = ["lean", "cloud", "backtest", PROJECT_ID,
           "--push", "--name", run_name]
...
# always write something so later steps know we ran
